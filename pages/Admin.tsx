...
import { X, Edit, Trash2, LayoutList, BookOpen, Award, Share2, Copy, Check, Lock, ArrowRight, ShieldCheck, FileJson, Upload, Image as ImageIcon, Mail } from 'lucide-react';
...
const Admin: React.FC = () => {
  ...
  const handleCocktailSubmit = (e: React.FormEvent) => {
      ...
      cocktailForm.id ? updateCocktail(payload) : addCocktail({ ...payload, id: crypto.randomUUID() });
  ...
  const handleTheorySubmit = (e: React.FormEvent) => {
      ...
      theoryForm.id ? updateTheory(payload) : addTheory({ ...payload, id: crypto.randomUUID() });
  ...
  const handleCertSubmit = (e: React.FormEvent) => {
      ...
      certForm.id ? updateCertificate(payload) : addCertificate({ ...payload, id: crypto.randomUUID() });
             ...
             <div className="bg-white/80 dark:bg-black/40 backdrop-blur-2xl border border-white/50 dark:border-white/10 rounded-[2.5rem] shadow-2xl dark:shadow-black/50 p-8 md:p-12">
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                    <div className="relative">
                        ...
                        <input 
                            type="email" 
                            className="w-full pl-12 pr-6 py-4 rounded-2xl bg-gray-50 dark:bg-black/50 border-2 border-transparent focus:border-brand-orange/50 dark:focus:border-brand-orange/50 text-gray-900 dark:text-white placeholder-gray-400 outline-none transition-all duration-300 font-medium" 
                            value={email} 
                    ...
                    {error && <div className="flex items-center justify-center gap-2 text-red-500 bg-red-50 dark:bg-red-900/20 py-2 rounded-xl animate-shake"><ShieldCheck size={16} /><span className="text-sm font-bold">{error}</span></div>}
                    
                    <button type="submit" disabled={isLoading} className="w-full group relative overflow-hidden py-4 rounded-2xl bg-gray-900 dark:bg-white text-white dark:text-black font-black text-lg transition-all duration-300 hover:shadow-lg hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed">
                        <span className="relative flex items-center justify-center gap-2">
                            {isLoading ? 'Accesso in corso...' : t.admin.loginButton} 
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-fadeIn">
      <div className="bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] shadow-xl border border-gray-100 dark:border-gray-800 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center relative z-10">
              ...
              <button onClick={logout} className="mt-4 md:mt-0 px-6 py-2 rounded-full border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 text-sm font-bold transition-colors">{t.admin.logout}</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
              <button onClick={() => { setActiveTab('cocktails'); setViewMode('edit'); setCocktailForm(emptyCocktail); }} className="group p-6 rounded-3xl bg-gray-50 dark:bg-black border border-gray-100 dark:border-gray-800 hover:border-brand-orange dark:hover:border-brand-orange transition-all text-left">
                  <div className="w-12 h-12 rounded-2xl bg-brand-orange text-white flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform"><LayoutList size={24} /></div>
                  <h3 className="font-bold text-lg dark:text-white">Nuovo Cocktail</h3>
              </button>
              <button onClick={() => { setActiveTab('theory'); setViewMode('edit'); setTheoryForm(emptyTheory); }} className="group p-6 rounded-3xl bg-gray-50 dark:bg-black border border-gray-100 dark:border-gray-800 hover:border-blue-500 dark:hover:border-blue-500 transition-all text-left">
                  <div className="w-12 h-12 rounded-2xl bg-blue-600 text-white flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform"><BookOpen size={24} /></div>
                  <h3 className="font-bold text-lg dark:text-white">Nuova Sezione</h3>
              </button>
              <button onClick={() => { setActiveTab('certificates'); setViewMode('edit'); setCertForm(emptyCert); }} className="group p-6 rounded-3xl bg-gray-50 dark:bg-black border border-gray-100 dark:border-gray-800 hover:border-purple-500 dark:hover:border-purple-500 transition-all text-left">
                  <div className="w-12 h-12 rounded-2xl bg-purple-600 text-white flex items-center justify-center mb-4 shadow-lg group-hover:scale-110 transition-transform"><Award size={24} /></div>
            ...
            {viewMode === 'list' && (
                <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden min-h-[500px]">
                    <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-end gap-2">
                        {activeTab === 'cocktails' && (<button onClick={() => { setBulkJson(JSON.stringify(data.cocktails, null, 2)); setViewMode('bulk'); }} className="px-4 py-2 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 text-xs font-bold hover:bg-gray-200 dark:hover:bg-gray-700 flex items-center gap-2"><FileJson size={14} /> JSON</button>)}
                ...
                <div ref={formRef} className="bg-white dark:bg-gray-900 rounded-3xl shadow-lg p-8 animate-slideDown">
                   ...
                   {activeTab === 'cocktails' && (
                        <form onSubmit={handleCocktailSubmit} className="space-y-6">
                            ...
                            <div className="space-y-4"><div className="flex gap-4 border-b border-gray-100 dark:border-gray-800 pb-2"><button type="button" onClick={() => setImageSource('url')} className={`text-sm font-bold ${imageSource === 'url' ? 'text-brand-orange' : 'text-gray-400'}`}>URL</button><button type="button" onClick={() => setImageSource('file')} className={`text-sm font-bold ${imageSource === 'file' ? 'text-brand-orange' : 'text-gray-400'}`}>Upload</button></div>{imageSource === 'url' && <input className="w-full p-3 bg-gray-50 dark:bg-black/50 rounded-xl outline-none dark:text-white" value={cocktailForm.image} onChange={e => setCocktailForm({...cocktailForm, image: e.target.value})} placeholder="https://..." />}{imageSource === 'file' && <div className="p-6 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl text-center cursor-pointer" onClick={() => fileInputRef.current?.click()}><input type="file" ref={fileInputRef} onChange={(e) => handleFileUpload(e, 'cocktail')} className="hidden" accept="image/*" /><ImageIcon className="mx-auto text-gray-400" /></div>}</div>
